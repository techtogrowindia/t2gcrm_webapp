...
const API_LIST = [
  ...
  },
  {
    ...
    actions: [
      ...
      { name: 'List Leads (GET)', method: 'GET', query: 'module=leads&ownerId=WORKSPACE_ID&actorId=TEAM_MEMBER_ID', notes: 'Pass actorId (teamMembers.id) OR userEmail so the server can apply Team-Permissions visibility (assigned-only / unassigned) for non-owner callers. Without either the server treats you as owner and returns ALL leads.', resp: { success: true, data: [{ id: '123', name: 'John Smith', stage: 'New' }] }, errors: [{ status: 400, error: 'Invalid or missing module' }, { status: 400, error: 'ownerId is required' }] },
