          ...
          <div style={{ padding: '4px 0', maxHeight: 320, overflowY: 'auto' }}>
            {!d ? (
              ...
              >
                ...
                {/* Same call / WhatsApp / SMS actions as the Leads list, so a
                    follow-up can be actioned from here without navigating.
                    stopPropagation on each: the row itself opens the lead, and
                    without it every dial would also fire that navigation. */}
                {it.phone && (
                  <span style={{ display: 'flex', gap: 5, flexShrink: 0 }} onClick={e => e.stopPropagation()}>
                    <a href={`tel:${it.phone}`} title={`Call ${it.phone}`} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 26, height: 26, borderRadius: '50%', background: '#eff6ff', color: '#2563eb', textDecoration: 'none' }}>
                    ...
                    <a href={`https://wa.me/${String(it.phone).replace(/\D/g, '')}`} target="_blank" rel="noreferrer" title="WhatsApp" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 26, height: 26, borderRadius: '50%', background: '#e8fdf0', color: '#16a34a', textDecoration: 'none' }}>
                    ...
                    <a href={`sms:${it.phone}`} title="SMS" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 26, height: 26, borderRadius: '50%', background: '#f5f3ff', color: '#7c3aed', textDecoration: 'none' }}>
                      <svg viewBox="0 0 24 24" width="13" height="13" stroke="currentColor" strokeWidth="2.5" fill="none" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path></svg>
                    </a>
                    {it.email && (
                      <a href={`mailto:${it.email}`} title="Email" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 26, height: 26, borderRadius: '50%', background: '#f3f4f6', color: '#4b5563', textDecoration: 'none' }}>
                        <svg viewBox="0 0 24 24" width="13" height="13" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline></svg>
                      </a>
                    )}
                  </span>
                )}
